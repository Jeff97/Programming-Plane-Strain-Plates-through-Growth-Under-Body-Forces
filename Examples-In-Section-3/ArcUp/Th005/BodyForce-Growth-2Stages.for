...
! 1. f[L]=0 Nsolve from the epsilon order equation
        f1 = (3*(1 + 2*h + 8*h**2)*Cos(L*Pi) - 
     &   3*(1 + 2*h + 8*h**2)*Cos(Pi*X) + 
     &   (3 + 4*h)*Pi*(L - X)*Sin(Pi*X))/(16.*h*(1 + 12*h)*L*Pi)
        ...
        Lambda1z1St2 = -0.0625*(16*h*L*Pi+8*Epsilon*f1*(1 + 2*h)*L*Pi+ 
     &  2*Epsilon*h*Cos(Pi*X) + Epsilon*Pi*(L - X)*Sin(Pi*X))/
     & (h*L)
...
! =================================================================
        ...
        Lambda1z1St1 = -0.0625*(16*h*L*Pi+8*Epsilon*f1*(1 + 2*h)*L*Pi+ 
     &  2*Epsilon*h*Cos(Pi*X) + Epsilon*Pi*(L - X)*Sin(Pi*X))/
     & (h*L)
