...
! TODO: for testing, for 1MPa, 2.117 too small while 2.118 too large
      F = TargetF*(TIME(1)**3.0)/TotalT
...
! 1. f[L]=0 Nsolve from the epsilon order equation
        f1 = -36.74421501765836 + 73.05822749928917*X - 
     &   36.461841829295295*X**2 + 0.6938572390876167*X**3 + 
     &   5.389614737967472*X**4 - 87.10311094229381*X**5 + 
     &   780.777778253154*X**6 - 4761.239468607391*X**7 + 
     &   20193.071053805386*X**8 - 59627.46233608972*X**9 + 
     &   118772.38064430286*X**10 - 142899.01381114672*X**11 + 
     &   59281.54183961659*X**12 + 84202.31654896388*X**13 - 
     &   97160.69418780172*X**14 - 53466.88649941664*X**15 + 
     &   96295.16102970934*X**16 + 58716.249961441965*X**17 - 
     &   81726.75175857887*X**18 - 81523.8434676146*X**19 + 
     &   50807.24940561974*X**20 + 103003.2968444565*X**21 - 
     &   4656.181227990584*X**22 - 107989.89540156075*X**23 - 
     &   44179.203104405955*X**24 + 96415.4865477996*X**25 + 
     &   80442.25529554448*X**26 - 92033.82966385013*X**27 - 
     &   91427.62647591856*X**28 + 156061.35937173702*X**29 - 
     &   76773.75566206814*X**30 + 13336.404212141926*X**31

        Df1 = 0.0

        DDf1 = 0.0

        Lambda1z0St2 = (1 + Epsilon*f1)*Sqrt(1 + X**2)
        Lambda2z0 = 1.0
        LambdaSz0 = 0.0

        ! =====================================

        Lambda1z1St2 = -0.125*(Epsilon*(L - X)*X*(1 + X**2) + 
     &   h*(8*L + Epsilon*Sqrt(1 + X**2)) + 
     &   8*Epsilon*f1*L*(h + (1 + X**2)**1.5))/(h*L*(1 + X**2))
...
! =================================================================
        ...
        Lambda1z0St1 = (1 + Epsilon*f1)*Sqrt(1 + X**2)

        Lambda1z1St1 = -0.125*(Epsilon*(L - X)*X*(1 + X**2) + 
     &   h*(8*L + Epsilon*Sqrt(1 + X**2)) + 
     &   8*Epsilon*f1*L*(h + (1 + X**2)**1.5))/(h*L*(1 + X**2))
...
!I cannot get total time from UMAT variable, please modify TotalT manually 
      ...
      IF ( (TIME(1)+DTIME) .LE. 0.5) THEN
        G11=1.0+(G11St1-1.0)*2.0*(TIME(1)+DTIME)/TotalT
      ELSE IF ( (TIME(1)+DTIME) .LE. 1.0) THEN
