...
! TODO: one should modify the number of elements according to the mesh
      Parameter (NElem=2000, NGauss=27)
...
C ELASTIC PROPERTIES
        RhoR = 1064 ! kg/m^3
        fZ = -9.8 ! N/kg, downward
        C0 = 0.5*PROPS(1)! Pa  C0=0.5*Gm
        h = 0.05/20.0 ! m (total thickness = 2h)
...
! 1. f[L]=0 Nsolve from the epsilon order equation
        f1 = (-3*(1 + 20*h*(-1 + 40*h))*(Cos(10*L*Pi) - Cos(10*Pi*X))+
     &       10*(-3 + 40*h)*Pi*(L - X)*Sin(10*Pi*X))/
     &       (1600.*h*(-1 + 120*h)*L*Pi)

        Lambda1z0St2 = ((1 + Epsilon*f1)*Pi)/2.
        Lambda2z0 = 1.0
        LambdaSz0 = 0.0

        ! =====================================

        Lambda1z1St2 = (160*h*L*Pi + 8*Epsilon*f1*(-1 + 20*h)*L*Pi - 
     &                  2*Epsilon*h*Cos(10*Pi*X) + 
     &                  Epsilon*Pi*(L - X)*Sin(10*Pi*X))/(16.*h*L)
...
! =================================================================
        ...
        Lambda1z0St1 = ((1 + Epsilon*f1)*Pi)/2.

        Lambda1z1St1 = (160*h*L*Pi + 8*Epsilon*f1*(-1 + 20*h)*L*Pi - 
     &                  2*Epsilon*h*Cos(10*Pi*X) + 
     &                  Epsilon*Pi*(L - X)*Sin(10*Pi*X))/(16.*h*L)
